...
def memory_usage(batch_size, model):
    ...
    if tf.__version__.split('.')[0] != '2':
        ...
        trainable_count = np.sum([tf.keras.utils.count_params(w) for w in model.trainable_weights])
        non_trainable_count = np.sum([tf.keras.utils.count_params(w) for w in model.non_trainable_weights])
    else:
        shapes_mem_count = 0
        for l in model.layers:
            single_layer_mem = 1
